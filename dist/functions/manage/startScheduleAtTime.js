...
exports.default = new forgescript_1.NativeFunction({
    ...
    async execute(ctx) {
        ...
        if (!this["isValidReturnType"](timeArg))
            return timeArg;
        const tzArg = await this["resolveUnhandledArg"](ctx, 2);
        if (!this["isValidReturnType"](tzArg))
            return tzArg;
        const idArg = await this["resolveUnhandledArg"](ctx, 3);
        if (!this["isValidReturnType"](idArg))
            return idArg;
        const timeStr = timeArg.value.trim();
        const tzStr = tzArg.value.trim().toUpperCase();
        const jobID = idArg.value.trim();
        if (!ctx.client.intervals)
            ctx.client.interval = new Map();
        if (ctx.client.intervals.has(jobID))
            return this.customError(`Schedule with name ${jobID} already exists!`);
        ...
        if (tzStr.startsWith("UTC") || tzStr.startsWith("GMT")) {
            ...
            if (isNaN(num))
                return this.customError("Invalid timezone offset");
        ...
        const run = async () => {
            if (ctx.client.pausedSchedules.get(jobID))
                return;
...
//# sourceMappingURL=startScheduleAtTime.js.map